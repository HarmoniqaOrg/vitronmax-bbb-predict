  ...
  return () => {
    ...
    modPromise = (initModule as any)({
      ...
      locateFile: (file: string, scriptDirectory: string) => {
        console.log(`[RDKit locateFile] Called. Input file: "${file}", scriptDirectory: "${scriptDirectory}"`);
        if (file.endsWith(".wasm")) {
          ...
          const wasmUrl = `/assets/${file}`;
          console.log(`[RDKit locateFile] Returning for .wasm: "${wasmUrl}"`);
          return wasmUrl;
        }
        ...
        const fallbackUrl = scriptDirectory + file;
        console.log(`[RDKit locateFile] Returning for other file type: "${fallbackUrl}"`);
        return fallbackUrl;
      },
